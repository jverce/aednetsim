...
public class Router : public I_Intercambiador, private I_Enrutador
{
private:
	int m_i1Oct;
	int m_iCantHosts;
...
public:
	...
	void enviar()
	{
		...
		for (int cii = 0; cii < m_ColasVecinos.size(); cii++) {
			...
			for (int cij = 0; (cij < m_TablaEnrutamiento.getBW(iRouter)) || (iiterator_2 == m_ListaPaquetes.end()); iiterator_2++) {
		...
		while (iiterator != m_ColasVecinos.end()) {
			...
			for (int cii = 0; cii < (*iiterator).second.size(); cii++) {
				(*iiterator).first->recibir((*iiterator).second.push());
